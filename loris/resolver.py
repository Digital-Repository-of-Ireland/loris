...
class RadosHTTPResolver(SimpleHTTPResolver):
    '''Rados Gateway HTTP resolver that supports multiple configurable patterns for supported
    ...
    '''
    def __init__(self, config):
        super(RadosHTTPResolver, self).__init__(config)
    ...
    def format_from_ident(self, ident, potential_format):
        format = super(RadosHTTPResolver, self).format_from_ident(ident, potential_format)
        format = format.lower()
    ...
    def is_resolvable(self, ident):
        ...
        if len(keys) > 1:
            ...
            return super(RadosHTTPResolver, self).is_resolvable(ident)
